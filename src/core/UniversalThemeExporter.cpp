...
UniversalPalette UniversalThemeExporter::extractColors() {
  ...
  config
      ->reparseConfiguration(); // Force reload from disk to get latest changes
  ...
  if (Config::isMaterialYouOverrideEnabled()) {
    ...
    if (scheme != "MaterialYouLight" && scheme != "MaterialYouDark") {
      ...
      if (currentGlobal == ThemeReader::defaultDarkTheme() ||
        ...
        Logger::log("Universal Fallback: Deduced Night based on active "
                    "structural themes.",
                    Logger::Info);
        ...
        Logger::log("Universal Fallback: Deduced Day based on active "
                    "structural themes.",
                    Logger::Info);
      } else {
        // Absolute final fallback to solar clock
        Logger::log("Universal Fallback: Both colorscheme and structural "
                    "themes unrecognized. Falling back to Solar Clock.",
                    Logger::Warning);
    ...
    if (path.isEmpty()) {
      path = QDir::homePath() + "/.local/share/color-schemes/" + scheme +
             ".colors";
    }

    if (QFile::exists(path)) {
      Logger::log("UniversalThemeExporter: Sourcing colors from " + path,
                  Logger::Info);
      return extractColors(path);
    } else {
      Logger::log("UniversalThemeExporter: Failed to find " + scheme +
                      ".colors",
                  Logger::Warning);
    }
  }
...
UniversalPalette
UniversalThemeExporter::extractColors(const QString &configPath) {
...
UniversalPalette
UniversalThemeExporter::extractColorsFromConfig(KSharedConfig::Ptr config) {
  ...
  // Extract Colors
  palette.windowBg =
      windowScheme.background(KColorScheme::NormalBackground).color();
  ...
  palette.accent =
      selectionScheme.background(KColorScheme::NormalBackground).color();
  palette.selection = selectionScheme.background(KColorScheme::NormalBackground)
                          .color(); // Typically accent, but explicit
  ...
  palette.titleBarBg =
      headerScheme.background(KColorScheme::NormalBackground).color();
  palette.titleBarFg =
      headerScheme.foreground(KColorScheme::NormalText).color();
  ...
  palette.ansiBlack = windowScheme.background(KColorScheme::NormalBackground)
                          .color(); // Darkest
  // 1: Red (Negative)
  palette.ansiRed = windowScheme.foreground(KColorScheme::NegativeText).color();
  // 2: Green (Positive)
  palette.ansiGreen =
      windowScheme.foreground(KColorScheme::PositiveText).color();
  // 3: Yellow (Neutral)
  palette.ansiYellow =
      windowScheme.foreground(KColorScheme::NeutralText).color();
  // 4: Blue (Focus/Active)
  palette.ansiBlue =
      selectionScheme.background(KColorScheme::NormalBackground).color();
  // 5: Magenta (ActiveText often used for focus) or just reuse Blue derivative?
  palette.ansiMagenta =
      windowScheme.foreground(KColorScheme::ActiveText).color();
...
bool UniversalThemeExporter::writeToFile(const QString &path,
...
bool UniversalThemeExporter::backupFile(const QString &path) {
  ...
  if (QFile::exists(backupPath))
    return true; // Backup already exists, don't overwrite original backup logic
...
bool UniversalThemeExporter::restoreFile(const QString &path) {
  ...
  if (file.exists()) {
    if (!file.remove()) {
      Logger::log("Failed to remove current file during restore: " + path,
                  Logger::Error);
      return false;
    }
  }
...
bool UniversalThemeExporter::exportToVSCodeJSON(
    const QString &path, const UniversalPalette &palette) {
  QFile file(path);
  ...
  // Update keys
  colors["titleBar.activeBackground"] = colorToHex(
      palette.titleBarBg.isValid() ? palette.titleBarBg : palette.windowBg);
  colors["titleBar.activeForeground"] = colorToHex(
      palette.titleBarFg.isValid() ? palette.titleBarFg : palette.windowFg);
...
void UniversalThemeExporter::syncAll() {
  ...
  if (Config::isObsidianSyncEnabled()) {
    ...
    if (!vault.isEmpty())
      exportToObsidian(palette, vault);
  }
...
bool UniversalThemeExporter::exportToFirefox(const UniversalPalette &palette) {
  ...
  QString cssContent =
      QString(":root {\n"
              ...
              "#navigator-toolbox { background-color: var(--plasma-window-bg) "
              "!important; }\n"
              ".tab-background[selected=\"true\"] { background-color: "
              "var(--plasma-accent) !important; color: #fff !important; }\n"
  ...
  QList<BrowserPath> browsers = {
      ...
      {"Zen (Flatpak)",
       QDir::homePath() + "/.var/app/app.zen_browser.zen/.zen"}};
  ...
  for (const auto &browser : browsers) {
    ...
    QStringList profiles =
        browserDir.entryList(QDir::Dirs | QDir::NoDotAndDotDot);
    for (const QString &profile : profiles) {
      // We look for profiles that are likely active (default or specific
      // naming) Or just try to inject into all found profiles that contain a
      // prefs.js or extensions Simpler: Just look for folder.

      QString chromeDir = browser.path + "/" + profile + "/chrome";
      // For Zen, it might share structure or differ. Docs say standard
      ...
      if (profile.contains("default") || profile.contains("release") ||
        ...
        if (writeToFile(chromeDir + "/plasma-colors.css", cssContent)) {
          ...
          if (userChrome.open(QIODevice::ReadWrite | QIODevice::Text)) {
            ...
            userChrome.close();
            success = true;
            Logger::log("Exported theme to " + browser.name +
                            " profile: " + profile,
                        Logger::Info);
          } else {
            // Create new
            ...
            success = true;
            Logger::log("Created theme for " + browser.name +
                            " profile: " + profile,
                        Logger::Info);
          }
        }
...
bool UniversalThemeExporter::exportToBetterDiscord(
    const UniversalPalette &palette) {
  if (!Config::isBetterDiscordSyncEnabled())
    return false;
  // Standalone theme in themes folder
  QString themePath =
      QDir::homePath() + "/.config/BetterDiscord/themes/PlasmaMaster.theme.css";
  ...
  if (Config::isBetterDiscordMaterialEnabled()) { // Using the same bool for
                                                  // backwards compatibility
    imports += "@import "
               "url(\"https://refact0r.github.io/midnight-discord/build/"
               "midnight.css\");\n";
  }
  ...
  QString css =
      QString("/**\n"
              ...
              " * @description A dynamic discord theme based on Midnight, "
              "synchronized with KDE Plasma.\n"
          ...
          .arg(imports)
          .arg(
              colorToHex(palette.windowBg)) // text-0 (text on colored elements)
          ...
          .arg(colorToHex(
              palette.error)); // accent-new (usually red for mute/deafen)

  backupFile(themePath);
  bool res = writeToFile(themePath, css);
  if (res)
    Logger::log("Exported theme to BetterDiscord: " + themePath, Logger::Info);
  return res;
}

bool UniversalThemeExporter::exportToObsidian(const UniversalPalette &palette,
  ...
  if (res)
    Logger::log("Exported theme to Obsidian: " + vaultPath, Logger::Info);
  return res;
}
...
bool UniversalThemeExporter::exportToKitty(const UniversalPalette &) {
  ...
  QList<ThemeTarget> targets = {
      {"Day", QDir::homePath() + "/.config/kitty/light-theme.auto.conf",
       ThemeReader::defaultLightTheme()},
      {"Night", QDir::homePath() + "/.config/kitty/dark-theme.auto.conf",
       ThemeReader::defaultDarkTheme()}};

  // Helper to generate content
  auto generateConfig = [](const UniversalPalette &p) -> QString {
    return QString("foreground %1\n"
                   "background %2\n"
        ...
        .arg(colorToHex(p.ansiBlack))
        .arg(colorToHex(p.ansiBlackBright))
        .arg(colorToHex(p.ansiRed))
        .arg(colorToHex(p.ansiRedBright))
        .arg(colorToHex(p.ansiGreen))
        .arg(colorToHex(p.ansiGreenBright))
        .arg(colorToHex(p.ansiYellow))
        .arg(colorToHex(p.ansiYellowBright))
        .arg(colorToHex(p.ansiBlue))
        .arg(colorToHex(p.ansiBlueBright))
        .arg(colorToHex(p.ansiMagenta))
        .arg(colorToHex(p.ansiMagentaBright))
        .arg(colorToHex(p.ansiCyan))
        .arg(colorToHex(p.ansiCyanBright))
        .arg(colorToHex(p.ansiWhite))
        .arg(colorToHex(p.ansiWhiteBright));
  };
  ...
  for (const auto &target : targets) {
    if (target.globalTheme.isEmpty()) {
      Logger::log("Skipping Kitty export for " + target.name +
                      ": No Global Theme set.",
                  Logger::Warning);
      continue;
    }
    ...
    if (Config::isMaterialYouOverrideEnabled()) {
      QString schemeName =
          (target.name == "Day") ? "MaterialYouLight" : "MaterialYouDark";
      colorsPath =
          QStandardPaths::locate(QStandardPaths::GenericDataLocation,
                                 "color-schemes/" + schemeName + ".colors");
      if (colorsPath.isEmpty()) {
        colorsPath = QDir::homePath() + "/.local/share/color-schemes/" +
                     schemeName + ".colors";
      }
    } else {
      QString schemeName =
          GlobalThemeManager::getColorSchemeFromGlobal(target.globalTheme);
      colorsPath = GlobalThemeManager::findColorSchemePath(schemeName);
      if (colorsPath.isEmpty()) {
        Logger::log("Could not find color scheme file for " +
                        target.globalTheme + " (" + schemeName +
                        "). Using current system colors as fallback.",
                    Logger::Warning);
      }
    }
    ...
    if (writeToFile(target.path, content)) {
      anySuccess = true;
      Logger::log("Exported Kitty config (" + target.name + "): " + target.path,
                  Logger::Info);
    }
  }
  ...
  // The user instruction said: "check the contents of the existing files for
  // how the file should be set up" and "rework the revert kitty logic to just
  // delete light and dark-theme.auto.conf" But we should also stop adding the
  // old include.
...
bool UniversalThemeExporter::exportToKonsole(const UniversalPalette &palette) {
  ...
  QString schemePath =
      ...
      "/konsole/PlasmaMaster.colorscheme";
  ...
  QString schemeContent = QString("[General]\n"
                              ...
                              .arg(rgb(palette.ansiBlack))
                              .arg(rgb(palette.ansiBlackBright))
                              .arg(rgb(palette.ansiRed))
                              .arg(rgb(palette.ansiRedBright))
                              .arg(rgb(palette.ansiGreen))
                              .arg(rgb(palette.ansiGreenBright))
                              .arg(rgb(palette.ansiYellow))
                              .arg(rgb(palette.ansiYellowBright))
                              .arg(rgb(palette.ansiBlue))
                              .arg(rgb(palette.ansiBlueBright))
                              .arg(rgb(palette.ansiMagenta))
                              .arg(rgb(palette.ansiMagentaBright))
                              .arg(rgb(palette.ansiCyan))
                              .arg(rgb(palette.ansiCyanBright))
                              .arg(rgb(palette.ansiWhite))
                              .arg(rgb(palette.ansiWhiteBright));
  ...
  QString profilePath =
      QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) +
      "/konsole/PlasmaMaster.profile";
  QString profileContent = "[Appearance]\n"
...
bool UniversalThemeExporter::restoreFirefox() {
  ...
  // Cleaning up "plasma-colors.css" inside profile is tricky without
  // re-scanning. For now, relies on restoreFile logic if the user has a backup
  // of userChrome.css. We can also try to delete plasma-colors.css if found.
...
bool UniversalThemeExporter::restoreKonsole() {
  ...
  Logger::log("Removed PlasmaMaster Konsole files. You may need to manually "
              "reset your default Konsole profile.",
              Logger::Warning);
...
QStringList UniversalThemeExporter::scanBetterDiscordImports() {
  ...
  for (const QString &filename : files) {
    if (filename == "PlasmaMaster.theme.css")
      continue; // Skip self
...
bool UniversalThemeExporter::exportToVencord(const UniversalPalette &palette) {
  ...
  QString themePath =
      QDir::homePath() + "/.config/Vencord/themes/PlasmaMaster.theme.css";
  QFileInfo fileInfo(themePath);
  if (!QDir(fileInfo.absolutePath()).exists()) {
    QDir().mkpath(fileInfo.absolutePath());
  }
...
bool UniversalThemeExporter::exportToBtop(const UniversalPalette &palette) {
  ...
  QString btopTheme = QString("# Theme generated by Plasma Theme Master\n"
                              "theme[main_bg]=\"%3\"\n"
                              "theme[main_fg]=\"%1\"\n"
                              "theme[title]=\"%1\"\n"
                              "theme[hi_fg]=\"%2\"\n"
  ...
  // Attempt to update btop configuration to use this theme if it's set to
  // something else
  QString confPath = QDir::homePath() + "/.config/btop/btop.conf";
  QFile confFile(confPath);
  if (confFile.exists() &&
      confFile.open(QIODevice::ReadWrite | QIODevice::Text)) {
...
bool UniversalThemeExporter::restoreBtop() {
  QString themePath =
      QDir::homePath() + "/.config/btop/themes/plasma-theme-master.theme";
  return QFile::remove(themePath);
}
...
bool UniversalThemeExporter::exportToVicinae(const UniversalPalette &palette) {
  ...
  if (writeToFile(themePath, vicinaeTheme)) {
    QProcess::startDetached("vicinae", QStringList() << "theme" << "set"
                                                     << "plasma-theme-master");
    Logger::log("Exported theme to Vicinae and triggered update.",
                Logger::Info);
    return true;
  }
  return false;
...
bool UniversalThemeExporter::restoreVicinae() {
  QString themePath = QDir::homePath() +
                      "/.local/share/vicinae/themes/plasma-theme-master.toml";
  return QFile::remove(themePath);
}
...
bool UniversalThemeExporter::exportToZed(const UniversalPalette &) {
  ...
  auto generateStyle = [](const UniversalPalette &palette,
                          bool isDark) -> QJsonObject {
    QJsonObject style;

    auto mix = [](QColor a, QColor b, float ratio) {
      return QColor(a.red() * ratio + b.red() * (1 - ratio),
    ...
    style["search.active_match_background"] =
        toHexAlpha(palette.ansiYellow, 100);
    ...
    style["editor.document_highlight.read_background"] =
        toHexAlpha(palette.ansiBlue, 32);
    style["editor.document_highlight.write_background"] =
        toHexAlpha(palette.ansiYellow, 32);
    ...
    style["version_control.conflict_marker.ours"] =
        toHexAlpha(palette.ansiGreen, 64);
    style["version_control.conflict_marker.theirs"] =
        toHexAlpha(palette.ansiBlue, 64);
    ...
    QJsonArray players;
    QList<QColor> pColors = {palette.ansiBlue,       palette.ansiRed,
                             palette.ansiYellow,     palette.ansiMagenta,
                             palette.ansiCyan,       palette.ansiRedBright,
                             palette.ansiBlueBright, palette.ansiGreen};
    ...
    // Syntax
    QJsonObject syntax;
    auto addSyntax = [&](const QString &key, QColor c,
                         const QString &fStyle = "", int fWeight = -1) {
      QJsonObject s;
      s["color"] = toHex(c);
      if (!fStyle.isEmpty())
        s["font_style"] = fStyle;
      else
        s["font_style"] = QJsonValue::Null;
      if (fWeight > 0)
        s["font_weight"] = fWeight;
      else
        s["font_weight"] = QJsonValue::Null;
      syntax[key] = s;
    };
  ...
  for (const auto &target : targets) {
    if (!Config::isMaterialYouOverrideEnabled() &&
        target.globalTheme.isEmpty()) {
      Logger::log(
          "Skipping Zed export for " + target.name +
              ": No Global Theme set and Material You override disabled.",
          Logger::Warning);
      continue;
    }
    ...
    t["name"] = QString("Plasma Master %1")
                    .arg(target.name == "Day" ? "Light" : "Dark");
  ...
  QRegularExpression themeRegex(R"("theme"\s*:\s*(?:"[^"]*"|\{[^{}]*\}))",
  ...
  if (content.contains(themeRegex)) {
    ...
    if (lastBrace != -1) {
      ...
      // Replace trailing comma with nothing (if it exists immediately before
      // the brace area) But realistically just inserting before the brace is
      // fine.
...
bool UniversalThemeExporter::restoreZed() {
  bool restored = false;
  QString themePath =
      QDir::homePath() + "/.config/zed/themes/plasma-master.json";

  if (QFile::exists(themePath) && QFile::remove(themePath))
    restored = true;
